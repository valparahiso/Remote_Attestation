...
void magic_random_init()
{
  ...
  ocall_print_buffer("\nInitializing the randomness source, NOT using real randomness: TEST ONLY\n");
  entropy_bytes = (unsigned char *)malloc(512);
  if (!entropy_bytes)
  {
    ocall_print_buffer("Failed to initialize the randomness source\n");
    EAPP_RETURN(EAPP_ERR_RET);
  }
  int i;
  for (i = 0; i < 512; i++)
  ...
  ocall_print_buffer("Randomness source correctly initialized\n");
}
